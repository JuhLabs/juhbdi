...
VERSION=$(node -e "console.log(require('$(cd "$(dirname "$0")" && pwd)/.claude-plugin/plugin.json').version)" 2>/dev/null || echo "1.8.1")
...
p "${SURFACE}  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${RST}"
blank
gradient_line "  888888          888      888888b.   8888888b. 8888888"  "$G1" "$G2" "$G3" "$G4" "$G5"
gradient_line "    \"88b          888      888  \"88b  888  \"Y88b  888  "  "$G1" "$G2" "$G3" "$G4" "$G5"
gradient_line "     888          888      888  .88P  888    888  888  "  "$G1" "$G2" "$G3" "$G4" "$G5"
gradient_line "     888 888  888 88888b.  8888888K.  888    888  888  "  "$G1" "$G2" "$G3" "$G4" "$G5"
gradient_line "     888 888  888 888 \"88b 888  \"Y88b 888    888  888  "  "$G1" "$G2" "$G3" "$G4" "$G5"
gradient_line "     888 888  888 888  888 888    888 888    888  888  "  "$G1" "$G2" "$G3" "$G4" "$G5"
gradient_line "     88P Y88b 888 888  888 888   d88P 888  .d88P  888  "  "$G1" "$G2" "$G3" "$G4" "$G5"
gradient_line "     888  \"Y88888 888  888 8888888P\"  8888888P\" 8888888"  "$G1" "$G2" "$G3" "$G4" "$G5"
gradient_line "   .d88P                                               "  "$G1" "$G2" "$G3" "$G4" "$G5"
gradient_line " .d88P\"                                                "  "$G1" "$G2" "$G3" "$G4" "$G5"
gradient_line " 888P\"                                                 "  "$G1" "$G2" "$G3" "$G4" "$G5"
blank
